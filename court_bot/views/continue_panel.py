...
class ContinueView(discord.ui.View):
    ...
    async def _on_force_end(self, interaction: discord.Interaction) -> None:
        ...
        if case.get("status") != STATUS_AWAITING_CONTINUE:
            await interaction.response.send_message("当前议诉不在‘待决定是否继续议诉’状态。", ephemeral=True)
            return
        ...
        await self.bot.repo.set_status(self.case_id, STATUS_AWAITING_JUDGEMENT, "管理强制结束议诉")
        ...
        if updated_case:
            ...
            try:
                ...
                if space is not None:
                    await space.send("【系统】管理强制结束议诉，进入裁决。")
            except Exception:
                pass

            await self.bot.enter_judgement(updated_case)

        await interaction.edit_original_response(content="已强制结束议诉，已进入裁决。")
    ...
    async def _handle(self, interaction: discord.Interaction, *, choice: str) -> None:
        ...
        if choice == "end" or state.get("complainant_choice") == "end" or state.get("defendant_choice") == "end":
            await self.bot.repo.clear_continue_state(self.case_id)
            await self.bot.repo.set_status(self.case_id, STATUS_AWAITING_JUDGEMENT, "议诉结束")
            updated_case = await self.bot.repo.get_case(self.case_id)
            ...
            await interaction.edit_original_response(content="已选择结束议诉，已进入裁决。")
            return
            ...
            if updated_case:
                ...
                try:
                    ...
                    if space is not None:
                        ...
                        await space.send(
                            content=f"【系统】双方同意继续议诉，进入第 {updated_case.get('current_round')} 轮。",
                            view=CourtView(bot=self.bot, case_id=self.case_id),
                        )
                except Exception:
                    pass

            await interaction.edit_original_response(content="双方已同意继续议诉。")
            return
